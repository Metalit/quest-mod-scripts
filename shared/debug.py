...
class DebugArgs:
    ...
    raw_addr = arg(
        ...
        "raw-address",
        str,
        help="analyze a single address instead of a backtrace (prints the output)",
    )
    symbols_dir = arg(
        "s",
        "symbols",
...
def get_best_tombstone(args: DebugArgs):
...
def get_log_stacktrace(args: DebugArgs):
...
def pull_trace(args: DebugArgs) -> str:
...
def analyze(args: DebugArgs, trace: str) -> str:
...
def output(args: DebugArgs, trace: str):
    if not args.no_analyze:
        trace = analyze(args, trace)
    args.out_file.write_text(trace)


def run_addrtoline(args: DebugArgs):
    # windows is .exe, mac/linux has no extension
    addrtoline = find_llvm_prebuilts() / "llvm-addr2line.exe"
    if not addrtoline.exists():
        addrtoline = find_llvm_prebuilts() / "llvm-addr2line"
    if not addrtoline.exists():
        log("Error:", addrtoline, "not found")
        exit(1)
    run(addrtoline, "-e", args.symbols_dir / get_so_name(), "-Cfip", args.raw_addr)
